...
def train_supervised(
        ...
        base_clf_lr,
        lr_decay_factor,
        unfreeze_schedule,  # Dictionary mapping epoch -> number of top blocks to unfreeze
        device,
        max_epochs=100,
    ...
    history = []
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=0.1, weight=class_weight)
    ...
    blocks = model.get_layer_groups()
    num_blocks = len(blocks)

    # Initialize custom LR Scheduler referencing updated optimizer groups dynamically
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epochs, eta_min=1e-6)
    
    lr_scheduler_epochs = list(unfreeze_schedule)
    for epoch in tqdm(range(1, max_epochs + 1), desc="Training Progress", position=0):
        # --- Progressive Unfreezing Logic ---
        if epoch in unfreeze_schedule:
            k = unfreeze_schedule[epoch]
            print(f"\n[Epoch {epoch}] Unfreezing the top {k} encoder block(s)...")
            
            # Unfreeze final norm layer
            if hasattr(model.context_encoder, "norm") and model.context_encoder.norm is not None:
                for p in model.context_encoder.norm.parameters():
            ...
            # Re-build optimization groups with discriminative learning rates
            model.update_optimizer_groups(optimizer, base_clf_lr=base_clf_lr, decay_factor=lr_decay_factor)
        ...
        # Save checkpoints
        if epoch % checkpoint_freq == 0 or epoch == max_epochs or epoch >= lr_scheduler_epochs[-1]:
        ...
        # Debug: print discriminative learning rates
