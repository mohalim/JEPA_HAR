...
class DilationConvBlock(nn.Module):
    def __init__(self, dim, dilation, drop_path=0.):
        super().__init__()
        # Depthwise convolution to capture temporal dependencies
        self.dwconv = nn.Conv1d(dim, dim, kernel_size=3, padding=dilation, dilation=dilation, groups=dim)
...
class ConvolutionalEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DilationConvBlock(embed_dim, d_rate) for d_rate in dilation_rates
        ])
