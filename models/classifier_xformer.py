...
class JEPAClassifier(nn.Module):
    ...
    def get_layer_groups(self):
        if hasattr(self.context_encoder, "transformer"):
            transformer_mod = self.context_encoder.transformer
            if hasattr(transformer_mod, "layers"):
                # nn.TransformerEncoder stores its sub-blocks inside a ModuleList named 'layers'
                return list(transformer_mod.layers)
            else:
                raise AttributeError("The encoder's 'transformer' attribute has no accessible sub-layers.")
    ...
    def update_optimizer_groups(self, optimizer, base_clf_lr, decay_factor=0.1):
        ...
        # 1. Base Classification Head Parameters Group
        clf_group = {
            **opt_defaults,
    ...
    def unfreeze_last_k_layers(self, k):
        ...
        encoder_layers = self.context_encoder.transformer.layers
