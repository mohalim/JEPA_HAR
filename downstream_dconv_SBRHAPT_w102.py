...
from models.jepa_dconv import JEPA_SEQ
...
if not is_winseq:
    ...
    checkpoint_dir = f"checkpoints/w{window_size}/win_seq"
...
checkpoint_file = 'cpt_epoch70_w102_edim384.pt'
...
if is_train:
    ...
    if not is_stage_two:
        checkpoint_clf_dir = f"checkpoints_clf/w{window_size}/stage1"
        clf_lr = 2e-3
        
        optimizer = torch.optim.AdamW(classifier_model.parameters(), lr=clf_lr, betas=(0.9, 0.99))

    else:
        checkpoint_clf_dir = f"checkpoints_clf/w{window_size}/stage1" # to load best model from stage1
        enc_lr = 3e-4
        clf_lr = 1e-3
        
        checkpoint_clf_file = 'best_model_stage1_epoch67.pt'
        classifier_model.load_state_dict(torch.load(os.path.join(checkpoint_clf_dir, checkpoint_clf_file),
                                                    weights_only=True))
        
        checkpoint_clf_dir = f"checkpoints_clf/w{window_size}/stage2" # for saving best model in stage2
...
if checkpoint_clf_dir is None:
    if not is_stage_two:
        ...
        checkpoint_clf_dir = f"checkpoints_clf/w{window_size}/stage2"
